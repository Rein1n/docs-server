...
Depending on the authentication method, passwords for each mount are encrypted and stored in the ownCloud database, using a long random secret key stored in `config.php`. This allows ownCloud to access the shares when the users who own the mounts are not logged in. This access will not work if the mount is session based, where passwords are not stored and are available only for the current active session. In the case where another user is granted access to this mount, he will see a red triangle with and exclamation mark on the bottom right of the mount icon identifying lack of access.
...
This documentation is a necessary prerequisite to setup Kerberos for the use with ownCloud.
...
This documentation is a necessary prerequisite to setup this authentication type.
...
There are special settings required when adding a mount point using the this authentication type.
====
--
...
Admins can define that mount points can be shared to all (default), or restrct it to individuals or groups. Note that sharing is not available for all authorization methods. For details please see the
xref:enterprise/external_storage/enterprise_only_auth.adoc#authentication-option-details[Enterprise-Only Authentication Options]. In case, you will see the following on the moint point additionally: 
+
...
. Finally, the mount point is created and will look like this as in the exampe below if a user has set it up:
+
...
The `Kerberos Server ID` to be entered as shown in the screen below...

// when need to describe in enterprise only auth the config.php settings for the krb ser id, see config.apps.sample
...
Note that this authentication method can only be used by ordinary users if the admin hands over the `Kerberos Server ID` created. Consider this as a very sensitive piece of information.
